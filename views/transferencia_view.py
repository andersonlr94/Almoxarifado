...
def tela_transferencia(page: ft.Page):
    ...
    btn_transferir = ft.ElevatedButton("Transferir", icon=ft.Icons.SEND)
    ...
    btn_carregar.on_click   = carregar     # async
    btn_limpar.on_click     = limpar
    btn_transferir.on_click = transferir   # pyautogui (sincrono)

    # ---------------------------
    # Layout superior: 2 colunas (esq/dir) + botões
    # ---------------------------
    topo = ft.Row(
        controls=[
            ft.Container(
            ...
            ft.Column(
                controls=[btn_carregar, btn_transferir, btn_limpar],
                spacing=8,
